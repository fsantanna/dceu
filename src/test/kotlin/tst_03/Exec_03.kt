...
class Exec_03 {
    ...
    fun kk_01_tup() {
        val out = test("""
            val CO = coro () {
                val i = 1
                do { yield(nil) thus {as it => [99,10,99]} }[1]
            }
            val co = coroutine(CO)
            resume co()
            println(resume co())
    ...
    fun kk_06_dic() {
        val out = test("""
            val CO = coro () {
                @[(yield(nil)thus{as it => :x},yield(nil)thus {as it => 10})]
            }
            val co = coroutine(CO)
            resume co()
            resume co()
    ...
    fun kk_07_set() {
        val out = test("""
            val CO = coro () {
                val t = [99,99,99]
                set t[yield(nil)thus {as it => 1}] = 10
                t[1]
            }
            val co = coroutine(CO)
            resume co()
    ...
    fun kk_08_call() {
        val out = test("""
            val CO = coro () {
                yield(nil) thus { as x => println } (
                    yield(nil) thus { as it => 1 },
                    yield(nil) thus { as it => 2 }
                )
            }
            val co = coroutine(CO)
            resume co()
