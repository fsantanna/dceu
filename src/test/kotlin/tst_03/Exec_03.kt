...
class Exec_03 {
    ...
    fun aa_05_yield_err() {
        val out = test("""
            resume (coro () {
                yield(nil) { as it => set it = nil }
            }) ()
        """)
        assert(out == "anon : (lin 3, col 39) : invalid set : destination is immutable\n") { out }
    }
    ...
    fun cc_14_coro_defer() {
        val out = test("""
            val T = coro () {
                ...
                yield(nil)  { as it =>nil }  ;; never awakes
            }
            resume (coroutine(T)) ()
            println(:end)
        """)
    ...
    fun ii_02_status() {
        val out = test("""
            val CO = coro () { yield(nil) { as it => nil } }
            val co = coroutine(CO)
            println(status(co))
            resume co()
            println(status(co))
    ...
    fun kk_06_dic() {
        val out = test("""
            val CO = coro () {
                @[(yield(nil){as it => :x},yield(nil){as it => 10})]
            }
            val co = coroutine(CO)
            resume co()
            resume co()
