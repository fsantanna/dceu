fun Coder.main (): String {
    ...
    """ // BLOCK
        ...
        CEU_RET ceu_block_set (CEU_Dyns* dyns, CEU_Dyn* dyn, int isperm) {
            switch (dyn->type) {
                ...
                case CEU_VALUE_TUPLE:
                    for (int i=0; i<dyn->Ncast.Tuple.its; i++) {
                        if (dyn->Ncast.Tuple.mem[i].type > CEU_VALUE_DYNAMIC) {
                            if (CEU_RET_THROW == ceu_block_set(dyns, dyn->Ncast.Tuple.mem[i].Dyn, isperm)) {
                                return CEU_RET_THROW;
                            }
                        }
                    }
                ...
                case CEU_VALUE_VECTOR:
                    if (dyn->Ncast.Vector.type > CEU_VALUE_DYNAMIC) {
                        ...
                        for (int i=0; i<dyn->Ncast.Vector.its; i++) {
                            if (CEU_RET_THROW == ceu_block_set(dyns, *(CEU_Dyn**)(dyn->Ncast.Vector.mem + i*sz), isperm)) {
                                return CEU_RET_THROW;
                            }
                        }
                    }
                ...
                case CEU_VALUE_DICT:
                    for (int i=0; i<dyn->Ncast.Dict.max; i++) {
                        if ((*dyn->Ncast.Dict.mem)[i][0].type > CEU_VALUE_DYNAMIC) {
                            if (CEU_RET_THROW == ceu_block_set(dyns, (*dyn->Ncast.Dict.mem)[i][0].Dyn, isperm)) {
                                return CEU_RET_THROW;
                            }
                        }
                        if ((*dyn->Ncast.Dict.mem)[i][1].type > CEU_VALUE_DYNAMIC) {
                            if (CEU_RET_THROW == ceu_block_set(dyns, (*dyn->Ncast.Dict.mem)[i][1].Dyn, isperm)) {
                                return CEU_RET_THROW;
                            }
                        }
                    }
            ...
            } else if (dyn->up_dyns.dyns->up_block==NULL || (!dyn->isperm && dyns->up_block->depth < dyn->up_dyns.dyns->up_block->depth)) {
            ...
            for (int i=0; i<n; i++) {
                ...
                if (cur->type>CEU_VALUE_DYNAMIC && cur->Dyn->up_dyns.dyns->up_block!=NULL) {
                    if (max < cur->Dyn->up_dyns.dyns->up_block->depth) {
                        ...
                        hld = cur->Dyn->up_dyns.dyns->up_block;
                    }
                }
            }
            if (hld != NULL) {
                ceu_hold_add(&hld->dn_dyns, dyn);
            }
        }
        ...
        void ceu_vector_set (CEU_Dyn* vec, int i, CEU_Value v) {
            if (v.type == CEU_VALUE_NIL) {           // pop
                ...
                if (i == vec->Ncast.Vector.its) {           // push
                    ...
                    ceu_gc_inc(&v);
                    vec->Ncast.Vector.its++;
                    vec->Ncast.Vector.mem[sz*vec->Ncast.Vector.its] = '\0';
                } else {                            // set
                    assert(CEU_RET_RETURN == ceu_vector_get(vec, i));
                    ceu_gc_inc(&v);
                    ceu_gc_dec(&ceu_acc, 1);
    ...
    """ // CREATES
        CEU_Dyn* ceu_proto_create (CEU_Block* hld, int isperm, int type, CEU_Proto proto) {
            ...
            *ret = (CEU_Dyn) {
            ...
            assert(CEU_RET_RETURN == ceu_block_set(&hld->dn_dyns, ret, isperm));
            return ret;
        }
        ...
        CEU_Dyn* ceu_tuple_create (CEU_Block* hld, int n) {
            ...
            assert(ret != NULL);
            *ret = (CEU_Dyn) {
                CEU_VALUE_TUPLE, {NULL,-1}, NULL, 0, {
                    .Ncast = { 0, {.Tuple={n,{}} } }
                }
            };
            memset(ret->Ncast.Tuple.mem, 0, n*sizeof(CEU_Value));
            assert(CEU_RET_RETURN == ceu_block_set(&hld->dn_dyns, ret, 0));
            return ret;
        }
        ...
        CEU_Dyn* ceu_vector_create (CEU_Block* hld) {
            ...
            mem[0] = '\0';
            *ret = (CEU_Dyn) {
                CEU_VALUE_VECTOR, {NULL,-1}, NULL, 0, {
        ...
        CEU_RET ceu_coros_create (CEU_Block* hld, int max, CEU_Value* ret) {
            ...
            *ret = (CEU_Value) { CEU_VALUE_COROS, {.Dyn=coros} };
            ...
            assert(CEU_RET_RETURN == ceu_block_set(&hld->dn_dyns, coros, 1));  // 1=cannot escape this block b/c of tasks
        ...
        CEU_RET ceu_coro_create (CEU_Block* hld, CEU_Value* task, CEU_Value* ret) {
            ...
            *ret = (CEU_Value) { CEU_VALUE_CORO, {.Dyn=coro} };
            ...
            assert(CEU_RET_RETURN == ceu_block_set(&hld->dn_dyns, coro, 1));  // 1=cannot escape this block b/c of upvalues
        ...
        CEU_Dyn* ceu_track_create (CEU_Dyn* coro, CEU_Value* ret) {
            ...
            *trk = (CEU_Dyn) {
            ...
            CEU_Block* hld = (coro->Bcast.Coro.up_coros == NULL) ? coro->up_dyns.dyns->up_block : coro->Bcast.Coro.up_coros->up_dyns.dyns->up_block;
            assert(CEU_RET_RETURN == ceu_block_set(&hld->dn_dyns, trk, 1));
